layui.use(['form','layer','jquery','laypage','laydate'],function(){
    ...
    aviosyLeftClick();
    $('.userRecordDD').addClass('layui-this');
	...
	function searchBtn(pageNumber) {
        ...
        var url = SERVER_ADDR + "/customerService/getPatientNewAccess.json";
        var data = {};
        ...
        if(data.length != 0){
            for(var i=0;i<data.length;i++){
                dataHtml += '<tr>'
                    ...
                    +'<td style="color: #1E9FFF;cursor: pointer" onclick="news_reset_recordhis(this)" valueid="'+data[i].advisoryId+'" patientid="'+data[i].patientId+'" valuemobile="'+data[i].mobile+'" valueusername="'+data[i].name+'">'+data[i].name+'</td>'
                    +'<td>'+data[i].mobile+'</td>'
                  /*  +'<td>'+data[i].storeName+'</td>'*/
                    +'<td title="'+data[i].disease+'">'+returnSubstring(data[i].disease)+'</td>'
                    if(data[i].salesmanName){
                        dataHtml += '<td>'+data[i].salesmanName+'</td>'
                    }else if(data[i].customerServiceName){
                        dataHtml += '<td>'+data[i].customerServiceName+'</td>'
                    }else{
                        dataHtml += '<td>'+data[i].hospitalName+'</td>'
                    }
                   /* +'<td>'+data[i].allotHospitalName+'</td>'*/
                dataHtml +='<td>'+recordStatusOnlyOne(data[i].hospitalStatus,data[i].platformStatus)+'</td>'
                    +'<td>'+data[i].brief+'</td>'
...
function news_reset_recordhis(obj) {
    ...
    var elThis = $(obj);
    var index = layui.layer.open({
        title : username + "的所有回访",
        type : 2,
        area:['85%','85%'],
        content : "userRecordDetail.html?id=" + id + '&mobile=' + mobile + '&v=222',
        success : function(layero, index){
