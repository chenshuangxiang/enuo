   ...
   /* if(getQueryString('action') == 'reset'){
	...
	function addTable(retInfo) {
 	...
 	form.on("submit(addNews)",function(data){
        ...
        }else if(backZi == 'unConnect' || backZi == 'refuse'){
    ...
    /*    addNews.advisoryId = getQueryString('id');
       ...
       /* if(getQueryString('action') == 'reset'){
        ...
        /*var url = SERVER_ADDR + "/customerService/addAccess";
    ...
    function repulseAdvisory() {
        var addNews = {};
        addNews.advisoryId = getQueryString('id');
        addNews.brief = $('.allotStatus option:selected').text();
        console.log(addNews);
        var url = SERVER_ADDR + "/customerService/repulseAdvisory";
        ajaxGetRetInfo(url,addNews,function (retInfo) {
            if(retInfo.success){
                top.layer.msg(retInfo.data);
                setTimeout(function () {
