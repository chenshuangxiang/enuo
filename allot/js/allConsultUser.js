...
function searchBtn(pageNumber,orderType) {
    ...
    var url = SERVER_ADDR + "/customerService/getPatientAdvisory.json";
    var data = {};
    ...
    if(data.length != 0){
        for(var i=0;i<data.length;i++){
            dataHtml += '<tr onclick="backColor(this)">'
                ...
                +'<td style="color: #1E9FFF;cursor: pointer" valuemobile="'+data[i].mobile+'" valuepatientId="'+data[i].patientId+'" valueusername="'+data[i].name+'"  onclick="thisUserAdviosy(this)">'+data[i].name+'</td>'
                +'<td>'+data[i].mobile+'</td>'
...
function thisUserAdviosy(obj) {
    //var indexOpen = top.layer.msg('',{icon: 16,time:false,shade:0.8});
    var username = $(obj).attr('valueusername');
    var mobile = $(obj).attr('valuemobile');
    var id = $(obj).attr('valuepatientId');
    var elThis = $(obj);
    var index = layui.layer.open({
        title : username + "的所有咨询",
        type : 2,
        area:['85%','85%'],
        content : "allConsultUserDetail.html?id=" + id + '&mobile=' + mobile + '&v=222',
        success : function(layero, index){
            //$('.news_content tr').css('background-color','white');
