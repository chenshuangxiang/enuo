        ...
        if(data.length != 0){
            for(var i=0;i<data.length;i++){
                dataHtml += '<tr>'
                 ...
                 /*  +'<td><input type="checkbox" name="checked" lay-skin="primary" lay-filter="choose"></td>'*/
                    +'<td style="color: #1E9FFF;cursor: pointer" onclick="allot_record(this)" data_id="'+data[i].id+'" value_id="'+data[i].advisoryId+'" value_mobile="'+data[i].mobile+'" patient_id="'+data[i].patientId+'">'+data[i].username+'</td>'
                    +'<td>'+data[i].mobile+'</td>'
...
function allot_record(obj) {
    //历史回访
        ...
        var index = layui.layer.open({
            ...
            type : 2,
            area:['83%','83%'],
            content : "historyRecordShuju.html?id=" + id + '&mobile=' + mobile + '&patientid='+patientid + '&dataid='+dataid  + '&v=333',
            success : function(layero, index){
                setTimeout(function () {
