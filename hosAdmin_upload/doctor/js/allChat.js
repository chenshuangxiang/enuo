        ...
        if(data.length != 0){
            for(var i=0;i<data.length;i++){
                ...
                dataHtml +=  /*'<a class="layui-btn layui-btn-normal layui-btn-mini news_record_add" style="display: none" valueid="'+data[i].id+'"><i class="layui-icon">&#xe650;</i> 回访</a>' +*/
                    '</td>' +
                    '<td><a class="layui-btn layui-btn-normal layui-btn-mini news_reset_dohis" onclick="chats_record(this)" valueid="'+data[i].userId+'" valuenickname="'+data[i].nickname+'"><i class="iconfont icon-edit"></i>回复</a>' +
                    '</td>'
                    +'</tr>';
            }
        }else{
...
function chats_record(obj) {
    ...
    var index = layui.layer.open({
        ...
        type : 2,
        area:['600px','556px'],
        content : "chatDetail.html?valueid=" + id + '&nickname'+nickname,
