...
layui.use(['form','layer','jquery','laypage','upload','table'],function(){
    ...
    upload.render({
        ...
        ,url: SERVER_ADDR + '/hospital/materialTemplate/import'
        ,accept: 'file' //普通文件
        ,done: function(res){
            console.log(res)
            if(res.success == true){
   ...
   /* form.on('select(newsHos)', function(data){  //根据医院获取病种
    ...
    function updateDiscount(ids,number) {
        var url = SERVER_ADDR + "/hospital/materialTemplate/updateDiscount";
        var data = {};
        data.ids = ids;
	...
	function searchBtn(pageNumber) {
        ...
        var url = SERVER_ADDR + "/hospital/materialTemplate/getList.json";
        var data = {};
    ...
    function tableRender(retInfo) {
        layui.table.render({
            ...
            ,cols: [[ //标题栏
                ...
                ,{field: 'modelNo', title: '品名', width: 300,edit: 'text'}
    ...
    $('.typeSelect').change(function () {
        ...
        ajaxGetRetInfo(SERVER_ADDR + '/hospital/materialTemplate/getDetail.json',{id:id},function (retInfo) {
            console.log(retInfo)
            if(retInfo.success){
                var addNews = {};
...
function delTemplate(id,callback) {
    ...
    layer.confirm('确认删除？', {
        ...
        var url = SERVER_ADDR + "/hospital/materialTemplate/delete";
        var data = {};
        data.id = elThisid;
        ajaxGetRetInfo(url,data,function (retInfo) {
            console.log(retInfo)
