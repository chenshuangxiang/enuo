...
layui.use(['form','layer','jquery','layedit','laydate','upload'],function(){
	...
	**/
    ...
    if(getQueryString('action') == 'reset'){
        ajaxGetRetInfo(SERVER_ADDR + '/hospital/doctor/getDetail.json',{id:getQueryString('valueid')},function (retInfo) {
            console.log(retInfo)
			if(retInfo.success){
	...
	function addTable(retInfo) {
 	...
 	form.on("submit(addDoc)",function(data){
        ...
        if(getQueryString('action') == 'reset'){
            ...
            var url = SERVER_ADDR + "/hospital/doctor/add";
		}
