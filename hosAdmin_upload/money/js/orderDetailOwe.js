            ...
            if(retInfo.success){
               ...
               /* if(retInfo.data.user.subsist != 0){
                ...
                if(retInfo.data.receiptRecord.card){
                    $('.layer_notice').append('<p><span>POS机支付：￥</span>'+retInfo.data.receiptRecord.card+'元</p>');
                }
                    ...
                    if(retInfo.data.receiptDetail.length != 0){
                        retInfo.data.receiptDetail.forEach(function (value) {
                            ...
                            if(value.payMethod == 'cash'){
                                ...
                                $('.layer_notice').append('<p>'+new Date(value.createDate).Format('yyyy-MM-dd hh:mm:ss')+'<span> POS机支付：￥</span>'+value.paidAmount+'元</p>');
