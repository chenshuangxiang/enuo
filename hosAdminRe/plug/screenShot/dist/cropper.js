...
(function (factory) {
  ...
  function toArray(obj, offset) {
    ...
    function canvastoArray(obj, offset) {
        var args = [];
...
//    var $this = $(this),
      ...
      if (typeof options === 'string' && $.isFunction((fn = data[options]))) {
        console.log($('.cropper-face').width())
          console.log($('.cropper-face').height())
          console.log(args)
          if(args[0].width != undefined){
              args[0].width = $('.cropper-face').width();
              args[0].height = $('.cropper-face').height();
          }else{
              //args[0] = 0.686 // 自己设置宽高
          }
        result = fn.apply(data, args); // 加入截图后图片宽高，跳到cropper的1333行
      }
    });
