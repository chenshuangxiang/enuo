...
var refresher = {
	...
	onScrolling:function(e,pullDownEl,pullUpEl,pullUpOffset){
		console.log(pullUpEl.querySelector('.pullUpLabel').innerHTML)
		if (e.y>-(pullUpOffset)) {
		...
		if (e.y > 0) {
			...
			if ( e.scrollerH < e.wrapperH && e.y < (e.minScrollY-pullUpOffset) || e.scrollerH > e.wrapperH && e.y< (e.maxScrollY - pullUpOffset) ) {
                if(pullUpEl.querySelector('.pullUpLabel').innerHTML == '全部加载完毕'){  //判断全部加载完了，下拉不加载， 上拉还是刷新
                    return;
                }
			...
			if (pullDownEl.className.match('flip')) {
			 ...
			 if (pullUpEl.className.match('flip')) {
				...
				pullUpEl.querySelector('.loader').style.display="block"
                 $('.loader').css('text-align','left').css('margin-left','41%');
