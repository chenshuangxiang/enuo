layui.use(['form','layer','jquery','laypage','upload'],function(){
   ...
   /* form.on('select(newsHos)', function(data){  //根据医院获取病种
	...
	function searchBtn(pageNumber) {
        ...
        var url = SERVER_ADDR + "/hospital/parentIllnessHistoryTemplate/all/hpiContent";
        var data = {};
        data.keyword = $(".newsName").val();
        data.subjectId = getQueryString('valueid');
        ...
        if(data.length != 0){
            for(var i=0;i<data.length;i++){
                ...
                dataHtml += '<tr>'
                    ...
                    +'<td>'+ noData(data[i].subSubjectName)+'</td>'
                    +'<td>'+ noData(data[i].hpiContent)+'</td>'
...
function resetTemplate(obj) {
    ...
    var index = layui.layer.open({
        ...
        type : 2,
        area:['450px','350px'],
        content : "addnowIllTemplate.html?valueid=" + id + '&action=reset',
        success : function(layero, index){
...
function delTemplate(obj) {
    ...
    layer.confirm('确认删除？', {
        ...
        var url = SERVER_ADDR + "/hospital/parentIllnessHistoryTemplate/delete/hpiContent/one";
        var data = {};
        data.hospitalSubSubjectHpiContentId = elThisid;
        ajaxGetRetInfo(url,data,function (retInfo) {
            console.log(retInfo)
            if(retInfo.success){
                localStorage.setItem('allPatientTemplate',$('.layui-laypage-curr em').eq(1).text());
