layui.use(['form','layer','jquery','element'],function(){
        ...
        if(data && data.length != 0){
            for(var i=0;i<data.length;i++){
                ...
                if(!data[i].name){
                    ...
                    dataHtml+='<td valueid="'+data[i].id+'" valuename="'+data[i].name+'" valueIndex="1"  valuetype="'+data[i].type+'" onclick="specialProductOpen(this)" style="color: #2299ee;cursor: pointer;">'+noData(data[i].name)+'</td>'
                }
                dataHtml+='<td>'+noData(data[i].hospital)+'</td>'
    ...
    $("body").on("click",".news_record_add",function(){
        ...
        var index = layui.layer.open({
            ...
            title : "患者回访信息添加",
            type : 2,
            content : "recordAddShuju.html?id=" +getQueryString('id') + '&v=1134',
