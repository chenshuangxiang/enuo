...
function searchBtn(pageNumber,dateType) {
    $('.sureTime').attr('day',dateType);//分页用
    var date = new Date();
    var firstDay;
    var lastDay;
    if(dateType == 'day'){
        ...
        $('#newsTimeEnd').val(getWeek().split('/')[1]);
    ...
    ajaxGetRetInfo(url,data,function (retInfo) {
        ...
        layer.close(index);
        if(retInfo.success){
            if(retInfo.data.ja && retInfo.data.ja.length > 0){
...
function renderDate(data){
    ...
    if(data.length != 0){
        for(var i=0;i<data.length;i++){
            ...
            dataHtml += '<tr>'
                +'<td>'+Number(i+1)+'</td>'
                +'<td style="color: #2299ee;cursor: pointer" valueid="'+data[i].hospitalUserId+'" valuemobile="'+data[i].mobile+'" valueindex="1" onclick="toUserInfo(this)">'+noData(data[i].fullName)+'</td>' //getUserDetail
            ...
            if(data[i].diseases){
                ...
                dataHtml+='<td>'+noData(data[i].diseases)+'</td>';
            }
            ...
            if(!data[i].paidAmount || data[i].paidAmount == 0){
                dataHtml+='<td>无</td>'
            }else{
