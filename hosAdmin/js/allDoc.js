layui.use(['form','layer','jquery','laypage','laydate','element'],function(){
   ...
   /* form.on('select(newsHos)', function(data){  //根据医院获取病种
	...
	function searchBtn(pageNumber) {
        ...
        var url = SERVER_ADDR + "/hospital/doctor/getList.json";
        var data = {};
        data.name = $(".newsName").val();
       // data.age = $(".age").val();
        data.doctorTypeId = $(".docLevel").val();
  ...
  /*  $("body").on("click",".news_reset_dohis",function(){
        ...
        var index = layui.layer.open({
            ...
            type : 2,
            area:['80%','80%'],
            content : "addDoc.html?valueid=" + id + '&action=reset',
            success : function(layero, index){
                localStorage.setItem('allDoc',$('.layui-laypage-curr em').eq(1).text());
                setTimeout(function () {
                    layui.layer.tips('点击此处返回医生列表', '.layui-layer-setwin .layui-layer-close', {
    ...
    //历史回访
    /*$("body").on("click",".news_reset_recordhis",function(){
        //var indexOpen = top.layer.msg('',{icon: 16,time:false,shade:0.8});
        ...
        var index = layui.layer.open({
            ...
            type : 2,
            area:['80%','80%'],
            content : "historyRecord.html?id=" + id + '&mobile=' + mobile + '&valueid='+valueid,
            success : function(layero, index){
                setTimeout(function () {
                    layui.layer.tips('点击此处返回咨询列表', '.layui-layer-setwin .layui-layer-close', {
                        tips: 3
            ...
            for(var i=0;i<data.length;i++){
                dataHtml += '<tr>'
                 ...
                 /*  +'<td><input type="checkbox" name="checked" lay-skin="primary" lay-filter="choose"></td>'*/
					...
					+'<td>'+data[i].subSubjectName+'</td>'
					+'<td>'+data[i].specialty+'</td>';
                if(data[i].enabled == true){
                    dataHtml += '<td><input type="checkbox" name="close" class="enabled" title="'+data[i].id+'" checked lay-skin="switch" lay-filter="enabled" lay-text="是|否"></td>';
                }else if(data[i].enabled == false){
                    dataHtml += '<td><input type="checkbox" name="close" class="enabled" title="'+data[i].id+'" lay-skin="switch" lay-filter="enabled" lay-text="是|否"></td>';
                }
               /* if(data[i].status == "unallot"){
                ...
                dataHtml +=  /*'<a class="layui-btn layui-btn-normal layui-btn-mini news_record_add" style="display: none" valueid="'+data[i].id+'"><i class="layui-icon">&#xe650;</i> 回访</a>' +*/
                    '</td>' +
                    '<td><a class="layui-btn layui-btn-normal layui-btn-mini news_reset_dohis" onclick="reset_doc(this)" valueid="'+data[i].id+'"  valuemobile="'+data[i].mobile+'" valueusername="'+data[i].name+'"><i class="iconfont icon-edit"></i>修改</a></td>' +
                  /*  '<a class="layui-btn layui-btn-normal layui-btn-mini news_reset_recordhis" onclick="reset_docexample(this)" valueid="'+data[i].id+'" valuetype="doctor">添加案例</a>' +
