            ...
            for(var i=0;i<data.length;i++){
                ...
                dataHtml += '<tr>'
                 ...
                 /*  +'<td><input type="checkbox" name="checked" lay-skin="primary" lay-filter="choose"></td>'*/
                    ...
                    +'<td style="color: #2299ee;cursor: pointer"  onclick="toUserInfo(this)" valueindex="1"   valuemobile="'+data[i].mobile+'">'+data[i].name+'</td>'
                    +'<td>'+data[i].mobile+'</td>'
                    +'<td>'+data[i].sex+'</td>'
                ...
                dataHtml +='<td>'+noDataKong(data[i].nextAccessEmployeeName)+'</td>';
                if(data[i].todayNeedAccess == true){
...
function allot_record(obj) {
    ...
    var index = layui.layer.open({
        ...
        type : 2,
        area:['83%','83%'],
        content : "historyRecord.html?id=" + id + '&mobile=' + mobile + '&patientid='+patientid + '&dataid='+dataid  + '&v=1134',
        success : function(layero, index){
            setTimeout(function () {
