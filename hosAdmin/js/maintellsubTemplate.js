layui.use(['form','layer','jquery','laypage','upload'],function(){
   ...
   /* form.on('select(newsHos)', function(data){  //根据医院获取病种
	...
	function searchBtn(pageNumber) {
        ...
        var url = SERVER_ADDR + "/hospital/complainTemplate/all/disease";
        var data = {};
...
function resetTemplate(obj) {
    ...
    var index = layui.layer.open({
        ...
        type : 2,
        area:['450px','350px'],
        content : "addmaintellTemplate.html?valueid=" + id + '&action=reset',
        success : function(layero, index){
...
function delTemplate(obj) {
    ...
    layer.confirm('确认删除？', {
        ...
        var url = SERVER_ADDR + "/hospital/complainTemplate/delete/disease/one";
        var data = {};
        data.hospitalDiseaseOfSubSubjectComplainId = elThisid;
        ajaxGetRetInfo(url,data,function (retInfo) {
            console.log(retInfo)
            if(retInfo.success){
                localStorage.setItem('allPatientTemplate',$('.layui-laypage-curr em').eq(1).text());
