layui.use(['form','layer','jquery','laypage','laydate'],function(){
	...
	function searchBtn(pageNumber) {
        ...
        var url = SERVER_ADDR + "/hospital/allot/getPlatformAccessList.json";
        var data = {};
     ...
     /*   data.age = $(".age").val();
        ...
        data.type = 'hospital';
        data.platformStatus = $('.allotStatus').val();
        ...
        if(data.length != 0){
            for(var i=0;i<data.length;i++){
                dataHtml += '<tr>'
                    ...
                    +'<td  valueid="'+data[i].advisoryId+'" patientid="'+data[i].patientId+'" valuemobile="'+data[i].mobile+'" valueusername="'+data[i].name+'">'+data[i].username+'</td>'
                    +'<td>'+data[i].mobile+'</td>'
