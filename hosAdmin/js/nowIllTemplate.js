layui.use(['form','layer','jquery','laypage','upload'],function(){
   ...
   /* form.on('select(newsHos)', function(data){  //根据医院获取病种
	...
	function searchBtn(pageNumber) {
        ...
        var url = SERVER_ADDR + "/hospital/parentIllnessHistoryTemplate/all";
        var data = {};
    ...
    $(".addTemplateOne").click(function(){
        ...
        var index = layui.layer.open({
            ...
            type : 2,
            area:['450px','350px'],
            content : "addnowIllTemplate.html",
        ...
        if(data.length != 0){
            for(var i=0;i<data.length;i++){
               ...
               /* if(data[i].status == "unallot"){
                ...
                dataHtml += '<td>' +
                    '<a class="layui-btn layui-btn-normal layui-btn-mini" onclick="resetTemplate(this)" valueid="'+data[i].subjectId+'"><i class="iconfont icon-edit"></i>修改</a>' +
                    '<a class="layui-btn layui-btn-danger layui-btn-mini" onclick="delTemplate(this)" valueid="'+data[i].subjectId+'"><i class="layui-icon">&#xe640;</i>删除</a>' +
                    '</td>'
                    +'</tr>';
            }
        }else{
...
function resetTemplate(obj) {
    ...
    var index = layui.layer.open({
        ...
        type : 2,
        area:['85%','85%'],
        content : "nowIllsubTemplate.html?valueid=" + id,
        success : function(layero, index){
...
function delTemplate(obj) {
    ...
    layer.confirm('确认删除？', {
        ...
        var url = SERVER_ADDR + "/hospital/parentIllnessHistoryTemplate/delete/one";
        var data = {};
        data.subjectId = elThisid;
        ajaxGetRetInfo(url,data,function (retInfo) {
            console.log(retInfo)
            if(retInfo.success){
                localStorage.setItem('allPatientTemplate',$('.layui-laypage-curr em').eq(1).text());
