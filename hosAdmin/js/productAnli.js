layui.use(['form','layer','jquery','laypage','laydate','element'],function(){
	...
	function searchBtn(pageNumber) {
        ...
        var data = {};
        data.type = 'product';
        data.specialProductName = $(".newsName").val();
	...
	$(".addDoc").click(function(){
        ...
        var index = layui.layer.open({
            ...
            type : 2,
            area:['83%','83%'],
            content : "addCase.html?type=product",
            success : function(layero, index){
                localStorage.setItem('allDoc',$('.layui-laypage-curr em').eq(1).text());
                setTimeout(function () {
                    layui.layer.tips('点击此处返回项目案例列表', '.layui-layer-setwin .layui-layer-close', {
                        tips: 3
                    });
                },500)
            ...
            for(var i=0;i<data.length;i++){
                ...
                dataHtml +='<td>';
                    data[i].images.forEach(function (value) {
                        dataHtml += '<img style="max-height: 75px;margin-right: 8px" src="'+SERVER_ADDR + value+'">';
                    })
                dataHtml +='</td>';
                dataHtml += '<td>'+data[i].specialProductName+'</td>'
...
function reset_doc(obj) {
    ...
    var index = layui.layer.open({
        ...
        type : 2,
        area:['80%','80%'],
        content : "addCase.html?id=" + id + '&type=product&action=reset',
        success : function(layero, index){
            localStorage.setItem('allDoc',$('.layui-laypage-curr em').eq(1).text());
