layui.use(['form','layer','jquery','laypage','laydate'],function(){
	...
	function searchBtn(pageNumber) {
        ...
        var url = SERVER_ADDR + "/hospital/reception/visitRecord/waitApprovedList.json";
        var data = {};
        ...
        if(data.length != 0){
            for(var i=0;i<data.length;i++){
                ...
                dataHtml += '<tr>'
                    ...
                    +'<td>'+data[i].mobile+'</td>';
                dataHtml += '<td>'+data[i].originalLongTimeDoctorName+'</td>'
					+'<td>'+data[i].originalLongTimeDoctorHelpmateName+'</td>'
                    +'<td>'+data[i].source+'</td>'
                    +'<td>'+data[i].beDistribution+'</td>'
                    +'<td>'+data[i].creator+'</td>'
                    +'<td>'+number+'</td>'
                    +'<td>'+data[i].approveType+'</td>';
                dataHtml +=  '<td>';
                dataHtml +=  '<a class="layui-btn layui-btn-normal layui-btn-mini firm" onclick="checkfirm(this)" check="true"  valueid="'+data[i].id+'"><i class="layui-icon" style="margin:0 -7px 0 -6px;">&#xe618;</i> 通过</a>' +
                    '<a class="layui-btn layui-btn-danger layui-btn-mini layui-btn-xs" onclick="checkfirm(this)" check="false" valueid="'+data[i].id+'"><i class="layui-icon" style="margin-right: 0;">&#x1006;</i>驳回</a>';
                dataHtml += '</td>'
                    +'</tr>';
            }
        }else{
...
function checkfirm(obj) {
    ...
    layer.confirm(checkZi, {
        ...
        var url = SERVER_ADDR + "/hospital/reception/visitRecord/approve";
        var data = {};
