	...
	**/
    ...
    form.on('select(allotStatus)', function(data){
        ...
        if(data.elem[data.elem.selectedIndex].title == 'hasAccess'){
            $('.recordFont').text('下次回访时间');
            $('#nextRecordTime').attr('placeholder','请选择下次回访时间');
            $('.nextRecordTimeDiv').show();
        }else if(data.elem[data.elem.selectedIndex].title == 'reservation'){
            $('.recordFont').text('预约时间');
            $('#nextRecordTime').attr('placeholder','请选择预约时间');
            $('.nextRecordTimeDiv').show();
	...
	function addTable(retInfo) {
 	...
 	form.on("submit(addNews)",function(data){
       /* addNews.username = $(".newsName").val();
        ...
        addNews.hospitalUserId = getQueryString('id');
       ...
       /* if(getQueryString('action') == 'reset'){
        ...
        var url = SERVER_ADDR + "/hospital/user/access";
