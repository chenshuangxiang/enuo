...
layui.use(['form','layer','jquery','layedit','laydate','upload'],function(){
	...
	**/
    ...
    if(getQueryString('action') == 'reset'){
        ajaxGetRetInfo(SERVER_ADDR + '/hospital/complainTemplate/disease/one',{diseaseId:getQueryString('valueid')},function (retInfo) {
            console.log(retInfo)
			if(retInfo.success){
	...
	function addTable(retInfo) {
 	...
 	form.on("submit(addDoc)",function(data){
        ...
        addNews.disease = $(".newsName").val();
 		console.log(addNews);
        if(getQueryString('action') == 'reset'){
        	addNews.hospitalDiseaseOfSubSubjectComplainId = getQueryString('valueid');
        }
        var url = SERVER_ADDR + "/hospital/complainTemplate/edit/disease";
