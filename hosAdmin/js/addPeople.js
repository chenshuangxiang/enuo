...
layui.use(['form','layer','jquery','layedit'],function(){
    ...
    form.on('radio(role)', function(data){
        ...
        if(data.value == 4){  //医生
            $('.sypsdDiv').hide()
            $('.newsNameClass').empty().append('<select name="doctor" class="doctor" lay-filter="doctor">' +
                '<option value="">请选择医生</option>' +
                '</select>');
            getDoctorSelect(form);
            //$('.newsName').val('').hide();
            //$('.doctor').next('.layui-form-select').show();
        }else{
            if(data.value == 5){ //收银
                $('.sypsdDiv').show();
            }else{
            ...
            if($('.newsName').val() == undefined || $('.newsName').val() == '') {
                $('.newsNameClass').empty().append('<input type="text" class="layui-input newsName" lay-verify="required" placeholder="请输入员工姓名">');
            }
            ...
            //form.render();
            //$('.newsName').show();
            //$('.doctor').next('.layui-form-select').hide();
        }
    });
    form.on('select(doctor)', function(data){
        newsNameVal = data.elem[data.elem.selectedIndex].title;
    });
	...
	**/
	...
	function addTable(retInfo) {
 	...
 	form.on("submit(addPeople)",function(data){
        ...
        addNews.enabled = use;
        if(roleVal == 4){
            addNews.doctorId = $('.doctor').val();
            addNews.fullname = newsNameVal;
        }else{
            addNews.fullname = $('.newsName').val();
        }
        if(roleVal == 5){
            addNews.paymentPassword = $(".sypsd").val() || 654321;
        }
       /* if($('.section').val() != '直属员工'){
