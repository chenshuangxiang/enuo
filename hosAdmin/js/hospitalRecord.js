layui.use(['form','layer','jquery','laypage','laydate'],function(){
    ...
    toAllFirstTopTab('.allRecordLi','.toRecordBtn');
    //getHos(form); //获取医院
	...
	function searchBtn(pageNumber) {
        ...
        var url = SERVER_ADDR + "/hospital/user/access/info/list";
        var data = {};
        ...
        if(data.length != 0){
            for(var i=0;i<data.length;i++){
                dataHtml += '<tr>'
                    ...
                    +'<td style="color: #1E9FFF;cursor: pointer" valueindex="1" onclick="toUserInfo(this)" valueid="'+data[i].advisoryId+'" patientid="'+data[i].patientId+'" valuemobile="'+data[i].mobile+'" valueusername="'+data[i].name+'">'+data[i].fullname+'</td>'
                    +'<td>'+data[i].mobile+'</td>'
                  /*  +'<td>'+data[i].storeName+'</td>'*/
                   ...
                   /* +'<td>'+data[i].allotHospitalName+'</td>'*/
                    +'<td>'+recordStatusOnlyOne(data[i].status)+'</td>'
               ...
               /* '<a class="layui-btn layui-btn-normal layui-btn-mini news_record" valueid="'+data[i].id+'" valuemobile="'+data[i].mobile+'" valueusername="'+data[i].username+'"><i class="layui-icon">&#xe60e;</i> 历史回访</a>'*/
                   ...
                   /* +'</td>'*/
                dataHtml += '<td><a class="layui-btn layui-btn-danger layui-btn-mini" valueid="'+data[i].id+'" onclick="delAccess(this)">删除</a></td>';
                dataHtml += '</tr>';
            }
        }else{
...
function delAccess(obj) {
    var id = $(obj).attr('valueid');
    var url = SERVER_ADDR + "/hospital/user/del/access/" + id;
    var Data = '';
    /*Data.id = id;*/
