layui.use(['form','layer','jquery','element'],function(){
	...
	//加载页面数据
    if(getQueryString('readqian') == 'true'){
        $('.rowAccess').hide();
        $('.qianInfo').text('欠款记录');
    }
    Get.firstConcult();
            ...
            for(var i=0;i<data.length;i++){
                ...
                if(getQueryString('readqian') == 'true'){
                    if(data[i].status ==  "loading"){
                        dataHtml = setHtml(dataHtml,data[i],number);
                    }
                }else{
                    dataHtml = setHtml(dataHtml,data[i],number);
                }
    ...
    function setHtml(dataHtml,data,number) {
        dataHtml += '<tr>'
        ...
        if(!data.name){
            dataHtml+='<td>'+noData(data.name)+'</td>'
        }else{
            dataHtml+='<td valueid="'+data.id+'" valuename="'+data.name+'" valueIndex="0"  valuetype="'+data.type+'" onclick="specialProductOpen(this)" style="color: #2299ee;cursor: pointer;">'+noData(data.name)+'</td>'
        }
        dataHtml+='<td>'+noData(data.hospital)+'</td>'
            +'<td>'+noData(data.doctor)+'</td>'
            +'<td>'+noData(data.doctorHelpmate)+'</td>'
        if(data.name && !data.source){
            dataHtml+='<td>e诺平台</td>'
        }else{
            dataHtml+='<td>'+noData(data.source)+'</td>'
        }
        dataHtml+='<td>'+noData(data.creator)+'</td>'
        if(data.repeat == true){
            dataHtml += '<td>复诊</td>'
        ...
        dataHtml += '<td>'+number+'</td>'
            +'<td>'+returnProjectVisitRecordStatus(data.status)+'</td>';
        if(data.userOffLineProjectId){
            dataHtml += '<td valueuserOfflineProject="'+data.userOffLineProjectId+'" onclick="allOrderDetail(this)" style="color: #2299ee;cursor: pointer;">查看清单</td>'
        }else{
            if(data.status == 'complete' && data.repeat == true){
                dataHtml += '<td onclick="fuzhen(this)" valuestatus="complete" valueid="'+data.id+'" style="color: #2299ee;cursor: pointer;">复诊情况</td>'
            }else{
                dataHtml += '<td>暂未开单</td>'
            }
        }
        dataHtml+='<td>'+noData(data.brief)+'</td>';
        dataHtml += '</tr>';
        return dataHtml;
    }
