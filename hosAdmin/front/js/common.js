...
var SERVER_ADDR = 'http://127.0.0.1';////'http://127.0.0.1';;////'http://127.0.0.1';//'http://127.0.0.1:7070';// /////;//'///;'//;////;//
var chatTime;
//订单--根据类型统计
function chooseType() {
    var totlePrice = 0;
        ...
        if(retInfo.success){
            ...
            retInfo.data.forEach(function (value) {
                ...
                if(value.enabled == true){
                    if(type == 'doctor'){
                        $('.fenTo').append('<option  title="'+value.id+'" value="'+value.id+'" titletype="'+type+'">'+value.name+'(医生)</option>');
                    }else if(type == 'doctorHelpmate'){
                        $('.fenTo').append('<option  title="'+value.id+'" value="'+value.id+'" titletype="'+type+'">'+value.name+'(现场咨询)</option>');
...
function getSubjectSelect() {
    var url = SERVER_ADDR + "/common/getSubjects.json";
    var Data = '';
    ajaxGetRetInfo(url,Data,function (retInfo) {
        console.log(retInfo)
        if(retInfo.success){
...
function getUserDetail(obj) {
    ...
    var readqian = $(obj).attr('valuereadqian');
    var title;
    if(readqian == 'true'){
        title = '欠款清单'
    }else{
    ...
    var index = layui.layer.open({
        ...
        type : 2,
        area:['85%','85%'],
        content : "historyRecord.html?id=" + id + '&readqian=' + readqian + '&mobile=' + mobile + '&v=1134',
        success : function(layero, index){
            /*setTimeout(function () {
