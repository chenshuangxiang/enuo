...
function searchBtn(pageNumber,type) {
    ...
    var url = SERVER_ADDR + "/hospital/reception/visitRecord/getStatisticsList.json";
    var data = {};
    ...
    ajaxGetRetInfo(url,data,function (retInfo) {
        ...
        if(retInfo.success){
            ...
            $('.totalHospitalPrice').text(0).text(retInfo.data.inHospitaAmount);
            $('.totalPlatformPrice').text(0).text(retInfo.data.platformAmount);
            $('.totalmedicalInsurancePrice').text(0).text(retInfo.data.medicalInsuranceAmount);
...
*/
            dataHtml += '<tr>'
                ...
                +'<td style="color: #2299ee;cursor: pointer" valueid="'+data[i].hospitalUserId+'" valuemobile="'+data[i].mobile+'" onclick="getUserDetail(this)">'+data[i].userName+'</td>'
                +'<td>'+data[i].mobile+'</td>';
            if(!data[i].type){
                dataHtml += '<td>无</td>'
            }else{
            ...
            if(!data[i].diseases){
                ...
                dataHtml += '<td valueuserOfflineProject="'+data[i].id+'" onclick="allOrderDetail(this)" style="color: #2299ee;cursor: pointer;">'+data[i].diseases+'</td>'
            }
            ...
            dataHtml +='<td>'+noData(data[i].reception)+'</td>'
               ...
               /* +'<td>'+number+'</td>'*/
                ...
                +'<td>'+noData(data[i].totalAmount - data[i].paidAmount)  +'</td>'
                if(!data[i].remarks){
...
function searchBtnOutExcel() {
    ...
    var index = layer.msg('查询中，请稍候',{icon: 16,time:false,shade:0.8});
    var url = SERVER_ADDR + "/hospital/reception/visitRecord/exportVisitRecordExcel";
    var data = {};
    data.keyword = $(".newsName").val();
    data.status = $('.status').val();
    data.beginDate = $('#newsTime').val();
