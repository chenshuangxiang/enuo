            ...
            for(var i=0;i<data.length;i++){
                ...
                /* if(data[i].status == "unallot"){
                    ...
                    dataHtml += '<td><a class="layui-btn layui-btn-normal layui-btn-mini" onclick="addwrite(this)" valueid="'+data[i].id+'"  valuemobile="'+data[i].mobile+'" valueusername="'+data[i].name+'"><i class="iconfont icon-edit"></i>添加入诊</a>' +
                        '<a class="layui-btn layui-btn-normal layui-btn-mini" onclick="jiuzhenhis_open(this)" valueid="'+data[i].id+'">就诊记录</a>' ;
                    if(data[i].debtSize > 0){
                        dataHtml +=  '<a class="layui-btn layui-btn-danger layui-btn-mini" onclick="getUserDetail(this)" valuemobile="'+data[i].mobile+'" valuereadqian="true" valueid="'+data[i].hospitalUserId+'">有欠款</a>' ;
                    }
                    dataHtml +='</td>'
   ...
   /* var Get = {
    ...
    function firstWriteConcultSuccess(retInfo) {
        ...
        if (retInfo.success == true) {
            ...
            if (retInfo.data.sourceStr == 'e诺平台') {
