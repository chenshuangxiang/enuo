...
function searchBtn(pageNumber,dateType) {
    ...
    var url = SERVER_ADDR + "/hospital/reception/visitRecord/getReservationList.json";
    var data = {};
...
function newsList(retInfo,totalCount,current){
    ...
    laypage.render({
        ...
        jump : function(obj,firstLoaded){
            ...
            console.log(obj)
            if (!firstLoaded) {
                searchBtn(obj.curr,$('.thatTimeChoose').attr('day'));
            }
        }
    })
}
        ...
        for(var i=0;i<data.length;i++){
            ...
            /*dataHtml +=  '<td>';
                if(data[i].status == "success"){
                    ...
                    if(!data[i].visitRecordId){
                        dataHtml +=  '已完成'
                    }else{
                        if(data[i].visitRecordStatus == 'wait'){
                            dataHtml +=  '<a class="layui-btn layui-btn-normal layui-btn-mini firm" onclick="reset_fen_appiontmentList_open(this)" valueid="'+data[i].id+'" visitrecordid="'+data[i].visitRecordId+'" doctorid="'+data[i].doctorId+'" doctorhelpmateid="'+data[i].doctorHelpmateId+'">修改分诊</a>'
                        }else if(data[i].visitRecordStatus == 'loading'){
                            dataHtml +=  '治疗中'
                        }else if(data[i].visitRecordStatus == 'leave'){
                            dataHtml +=  '弃诊'
                        }else{
                            dataHtml +=  '';
                        }
