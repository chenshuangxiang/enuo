...
layui.use(['form','layer','jquery','laypage','laydate'],function(){
    ...
    form.on('radio(alltongji)', function(data){
        ...
        if(data.value == '治疗费统计'){
            window.location.href = SERVER_ADDR + '/hosAdmin/finance/index.html' + '#paymentList.html';
        }
    });
    form.on('radio(yibaotongji)', function(data){
        console.log(data)
        searchBtn(1,data.value);
...
function searchBtn(pageNumber,payMethod) {
    var index = layer.msg('查询中，请稍候',{icon: 16,time:false,shade:0.8});
    var url = SERVER_ADDR + "/hospital/finance/getSceneRegistrationFeeStatistics.json";
    var data = {};
    ...
    ajaxGetRetInfo(url,data,function (retInfo) {
        ...
        if(retInfo.success){
            ...
            $('.totalCount').text(retInfo.data.payCount);
        }else{
...
*/
            dataHtml += '<tr>'
              ...
              /*  +'<td>'+data[i].reservationDate+'</td>'*/
                +'<td style="color: #2299ee;cursor: pointer" valuetotlePrice="'+data[i].amount+'"  valuedate="'+data[i].paymentDate+'" onclick="visitUserInfo(this)">'+data[i].paymentDate+'</td>'
       ...
       /*     if(data[i].type == "coupon"){
            ...
            dataHtml += '<td>'+noData(data[i].amount)+'<a style="position: absolute; right: 19px;top: 3px;" class="layui-btn layui-btn-normal layui-btn-mini news_reset_recordhis" onclick="readPayMtethods(this)" valueid="'+data[i].id+'" valuedate="'+data[i].paymentDate+'" valuetype="doctor">查看明细</a></td>'
...
function readPayMtethods(obj){
    ...
    console.log(4555)
    var index = layui.layer.open({
        title : date + "现场支付挂号费支付明细",
        type : 2,
        area:['450px','250px'],
        content : "paymentPayMethodsList.html?date=" + date + '&payMethod=' + payMethod + '&payUrl=/hospital/finance/getScenePayRegistrationFeeDetails.json' + '&v=1134',
        success : function(layero, index){
...
function visitUserInfo(obj){
    ...
    var index = layui.layer.open({
        ...
        type : 2,
        area:['90%','80%'],
        content : "paymentDayGuahaoList.html?date=" + elThis + '&payMethods='+ payMethod + '&payUrl=/hospital/finance/getOneDaySceneRegistrationFeeRecord.json&v=1134',
        success : function(layero, index){
            //localStorage.setItem('visit',$('.layui-laypage-curr em').eq(1).text());
