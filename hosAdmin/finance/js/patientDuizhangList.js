...
layui.use(['form','layer','jquery','laypage','laydate'],function(){
    ...
    toAllFirstTopTab('.duizhangLi','.toReservationBtn');
...
function searchBtn(pageNumber,type) {
    ...
    var url = SERVER_ADDR + "/hospital/finance/getReconciliation.json";
    var data = {};
    ...
    if(data.length != 0){
        for(var i=0;i<data.length;i++){
            ...
            dataHtml  +='<td>'+returnVisitRecordNumber(data[i].number)+'</td>'
                ...
                +'<td>'+noData(data[i].refundAmount)+'</td>'
                +'<td>'+data[i].paidAmount +'/'+ data[i].totalAmount+'</td>'
                +'<td>'+returnProjectReconciliationStatus(data[i].reconciliation) +'</td>'
            if(data[i].reconciliation == 'NO_RECONCILIATION' || data[i].reconciliation == 'RECONCILIATIONING'){
                if(data[i].paidAmount != 0){
                    dataHtml += '<td><a class="layui-btn layui-btn-normal layui-btn-mini" onclick="duizhangOpen(this)" valuearrears="'+data[i].arrears+'" orderid="'+data[i].orderId+'"  valueid="'+data[i].userOffLineProjectId+'" >对账</a></td>' ;
                }else{
                    dataHtml += '<td></td>';
                }
            }else{
...
function duizhangOpen(obj) {
    var id = $(obj).attr('valueid');
    var orderId = $(obj).attr('orderid');
    var content;
    if($(obj).attr('valuearrears') == 'true'){
        content = 'duizhangOpen.html?id=' + id + '&orderid=' + orderId;
    }else{
        content = 'duizhangOpen.html?id=' + id;
    }
