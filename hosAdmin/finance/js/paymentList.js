...
layui.use(['form','layer','jquery','laypage','laydate'],function(){
    ...
    form.on('radio(alltongji)', function(data){
        if(data.value == '挂号费统计'){
            window.location.href = SERVER_ADDR + '/hosAdmin/finance/index.html' + '#paymentGuahaoList.html';
        }
    });
...
function searchBtn(pageNumber,payMethod) {
    ...
    var url = SERVER_ADDR + "/hospital/finance/getScenePayCount.json";
    var data = {};
...
*/
            dataHtml += '<tr>'
              ...
              /*  +'<td>'+data[i].reservationDate+'</td>'*/
                +'<td style="color: #2299ee;cursor: pointer" valuetotlePrice="'+data[i].amount+'"  valuedate="'+data[i].date+'" onclick="visitUserInfo(this)">'+data[i].date+'</td>'
       ...
       /*     if(data[i].type == "coupon"){
            ...
            dataHtml += '<td>'+noData(data[i].amount)+'<a style="position: absolute; right: 19px;top: 3px;" class="layui-btn layui-btn-normal layui-btn-mini news_reset_recordhis" onclick="readPayMtethods(this)" valuedate="'+data[i].date+'" valueid="'+data[i].id+'" valuetype="doctor">查看明细</a></td>'
...
function visitUserInfo(obj){
    ...
    var index = layui.layer.open({
        ...
        type : 2,
        area:['90%','80%'],
        content : "paymentDayList.html?date=" + elThis + '&payMethods='+ payMethod + '&payUrl=/hospital/finance/getOneDayScenePaymentRecord.json&v=1134',
        success : function(layero, index){
...
function readPayMtethods(obj){
    ...
    console.log(4555)
    var index = layui.layer.open({
        title : date + "现场支付治疗费支付明细",
        type : 2,
        area:['450px','250px'],
        content : "paymentPayMethodsList.html?date=" + date + '&payMethod=' + payMethod + '&payUrl=/hospital/finance/getScenePayDetails.json' + '&v=1134',
        success : function(layero, index){
            //localStorage.setItem('visit',$('.layui-laypage-curr em').eq(1).text());
