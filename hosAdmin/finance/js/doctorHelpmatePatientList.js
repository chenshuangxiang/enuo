...
function searchBtn(pageNumber,type) {
    ...
    var url = SERVER_ADDR + "/hospital/finance/getDoctorHelpmateOrder.json";
    var data = {};
    data.doctorHelpmateId = getQueryString('id');
    ...
    if(data.length != 0){
        for(var i=0;i<data.length;i++){
            if(data[i].status == 'wait'){
                ...
                data[i].status = '<span style="color: #FFB800">挂号中</span>';
            }
            if(data[i].sex == 'man'){
                data[i].sex = '男';
       ...
       /*     if(data[i].type == "coupon"){
            ...
            dataHtml += '<td>'+number+'</td>' +
               ...
               /* +'<td>'+noData(data[i].source)+'</td>'
                ...
                +'<td>'+noPriceData(data[i].paidAmount) +'/'+ noPriceData(data[i].totalPrice)+'</td>'
                +'<td>'+noPriceData(data[i].lastAmount) +'</td>'
                if(data[i].id){
