...
function searchBtn(pageNumber,type) {
    ...
    var url = SERVER_ADDR + "/hospital/finance/getNurseStatistics.json";
    var data = {};
        ...
        for(var i=0;i<data.length;i++){
            if(data[i].enabled == false && data[i].todayTurnovers == 0 && data[i].monthTurnovers == 0){
                ...
                dataHtml += '<tr>'
                    ...
                    +'<td style="color: #2299ee;cursor: pointer" valueid="'+data[i].nurseId+'" onclick="visitUserInfo(this)">'+data[i].name+'</td>'
                    +'<td>护士</td>';
                dataHtml += '<td>'+noData(data[i].hospitalName)+'</td>'
                    +'<td>'+noData(data[i].todayTurnovers)+'</td>'
                    +'<td>'+noData(data[i].monthTurnovers)+'</td>'
                dataHtml += '</tr>';
            }
        }
    }else{
...
function visitUserInfo(obj){
    ...
    var index = layui.layer.open({
        ...
        type : 2,
        area:['90%','80%'],
        content : "nursePatientList.html?id=" + elThis,
        success : function(layero, index){
