...
layui.use(['form','layer','jquery','laypage','laydate'],function(){
    ...
    form.on('radio(alltongji)', function(data){
        ...
        if(data.value == '治疗费统计'){
            window.location.href = SERVER_ADDR + '/hosAdmin/finance/index.html' + '#paymentOnlineList.html';
        }
    });
...
function searchBtn(pageNumber,type) {
    ...
    var url = SERVER_ADDR + "/hospital/finance/getPlatformRegistrationFeeStatistics.json";
    var data = {};
...
*/
       ...
       /*     if(data[i].type == "coupon"){
            ...
            dataHtml += '<td>'+noData(data[i].amount)+'<a class="layui-btn layui-btn-normal layui-btn-mini news_reset_recordhis" onclick="readPayMtethods(this)" valuedate="'+data[i].paymentDate+'" valueid="'+data[i].id+'" valuetype="doctor">查看明细</a></td>'
...
function readPayMtethods(obj){
    ...
    console.log(4555)
    var index = layui.layer.open({
        title : date + "平台支付挂号费支付明细",
        type : 2,
        area:['450px','250px'],
        content : "paymentPayMethodsList.html?date=" + date + '&payUrl=/hospital/finance/getPlatformRegistrationFeePayDetails.json' + '&v=1134',
        success : function(layero, index){
...
function visitUserInfo(obj){
    ...
    var index = layui.layer.open({
        ...
        type : 2,
        area:['90%','80%'],
        content : "paymentDayGuahaoList.html?date=" + elThis + '&payUrl=/hospital/finance/getOneDayPlatformRegistrationFeePaymentRecord.json&v=1134',
        success : function(layero, index){
            //localStorage.setItem('visit',$('.layui-laypage-curr em').eq(1).text());
