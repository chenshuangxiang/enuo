...
layui.use(['form','layer','jquery','laypage','laydate'],function(){
    ...
    $('.doctorHelpmateList').addClass('layui-this').parent().parent().addClass('layui-nav-itemed');
	//加载页面数据
...
function searchBtn(pageNumber,type) {
    ...
    var url = SERVER_ADDR + "/hospital/finance/consultation/statistics";
    var data = {};
...
function renderDate(data){
    ...
    if(data.length != 0){
        for(var i=0;i<data.length;i++){
            if(data[i].enabled == false && data[i].todayOrderPay == 0 && data[i].monthOrderPaidPay == 0){

            }else{
                dataHtml += '<tr>'
                    +'<td>'+Number(i+1)+'</td>'
                    /*  +'<td>'+data[i].reservationDate+'</td>'*/
                    +'<td style="color: #2299ee;cursor: pointer" valueid="'+data[i].doctorHelpmateId+'" onclick="visitUserInfo(this)">'+data[i].fullname+'</td>'
                    +'<td>'+data[i].mobile+'</td>';
                dataHtml += '<td>'+noData(data[i].todayOrderPay)+'</td>'
                    +'<td>'+noData(data[i].monthOrderPaidPay)+'</td>'
                dataHtml += '</tr>';
            }
        }
    }else{
...
function visitUserInfo(obj){
    ...
    var index = layui.layer.open({
        ...
        type : 2,
        area:['90%','80%'],
        content : "doctorHelpmatePatientList.html?id=" + elThis,
        success : function(layero, index){
