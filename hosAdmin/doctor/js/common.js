...
var SERVER_ADDR = 'http://127.0.0.1';//////'http://127.0.0.1:7070';// /////;//'///;'//;////;//
var chatTime;
//订单--根据类型统计
function chooseType() {
    var totlePrice = 0;
...
function fuzhen(obj) {
    ...
    if($(obj).attr('valuestatus') == 'complete'){
        content = "fuzhen.html?valueid=" + elThis + '&action=reset&v=1134'
    }else{
        content = "fuzhen.html?valueid=" + elThis + '&v=1134'
    }
...
//品名
function getModelNo(form,type) {
    var url = SERVER_ADDR + "/hospital/doctor/index/getMaterialList.json";
    var Data = {};
    Data.type = type;
    ajaxGetRetInfo(url,Data,function (retInfo) {
        console.log(retInfo)
        if(retInfo.success){
            $('.cailiaoName').empty().append('<option value="">请选择品名(选择添加)</option>');
            retInfo.data.forEach(function (value) {
                $('.cailiaoName').append('<option value="'+value.id+'" title="'+value.id+'">'+value.modelNo+'</option>');
            });
            form.render();
...
//医保品名
function getMedicalModelNo(form,type) {
    var url = SERVER_ADDR + "/hospital/doctor/index/getMaterialMedicalcareList.json";
    var Data = '';
   // Data.type = type;
    ajaxGetRetInfo(url,Data,function (retInfo) {
        console.log(retInfo)
...
function getDiagnosesMuban(form) {
    var url = SERVER_ADDR + "/hospital/medicalRecordTemplate/getMedicalRecordTemplate.json";
    var Data = '';
    ajaxGetRetInfo(url,Data,function (retInfo) {
        console.log(retInfo)
        if(retInfo.success){
            retInfo.data.forEach(function (value) {
                $('.diagnosesMuban').append('<option value="'+value.id+'" title="'+value.id+'">'+value.name+'</option>');
            });
            form.render();//更新
        }
    },'请求失败', 'GET', undefined, undefined);
        ...
        if(retInfo.success){
            ...
            retInfo.data.forEach(function (value) {
                ...
                if(value.enabled == true){
                    if(type == 'doctor'){
                        $('.fenTo').append('<option value="'+value.id+'" title="'+type+'">'+value.name+'(医生)</option>');
                    }else if(type == 'doctorHelpmate'){
                        $('.fenTo').append('<option value="'+value.id+'" title="'+type+'">'+value.name+'(咨询)</option>');
                        ...
                        $('.nurse').append('<option value="'+value.id+'" title="'+type+'">'+value.name+'(护士)</option>');
                    }
                }
            });
            form.render();//更新
...
function getDoctorNameFrontSelect(type) {
    var url = SERVER_ADDR + "/hospital/employee/getListByRole.json";
    var Data = {};
    Data.type = type;
    ajaxGetRetInfo(url,Data,function (retInfo) {
        ...
        if(retInfo.success){
            //$('.fenToDoctor').empty().append('<option value="">请选择医生</option>');
                ...
                if(type == 'doctor'){
                    retInfo.data.forEach(function (value) {
                        if(value.enabled == true){
                            $('.fenToDoctor').append('<option value="'+value.id+'" title="cure">'+value.name+'(医生)</option>');
                        }
                    });
                }else if(type == 'medicalTechnology'){
                    retInfo.data.forEach(function (value) {
                        if(value.enabled == true){
                            $('.fenToDoctor').append('<option value="'+value.id+'" title="check">'+value.name+'(医技)</option>');
                        }
                    });
                }
           /* for(var i = 0; i < $('.fenToName').length;i++){
...
function getTemplateProject() {
    ...
    ajaxGetRetInfo(url,data,function (retInfo) {
        ...
        if(retInfo.success){
            ...
            if(retInfo.data != ''){
                retInfo.data.forEach(function (value) {
                    $('.projectName').append('<option value="'+value.id+'" title="'+value.id+'" valuetitle="'+value.name+'">'+value.name+'</option>');
                });
            }
            //form.render();//更新
            $('.projectName').comboSelect();
        }else{
            $('.projectName').comboSelect();
        }
    },'请求失败', 'GET', undefined, undefined);
}
function getMaintellInfo(keyword,bool) {
    var bool = bool;
    var url = SERVER_ADDR + "/hospital/complainTemplate/allList/distinct/disease";
    var data = {};
    ...
    ajaxGetRetInfo(url,data,function (retInfo) {
        ...
        if(retInfo.success){
            ...
            var html = '';
            retInfo.data.forEach(function (value) {
                html += '<a onclick="maitelladd(this)" class="layui-btn layui-btn-primary">'+value+'</a>'
            })
            $('.maintellUl p').html(html);
        }else{
            alert(retInfo.data);
        }
    },'请求失败', 'GET', undefined, undefined);
}
function getNowIllInfo(keyword,bool) {
    var bool = bool;
    var url = SERVER_ADDR + "/hospital/parentIllnessHistoryTemplate/allList/distinct/hpiContent";
    var data = {};
    data.keyword = keyword;
    ajaxGetRetInfo(url,data,function (retInfo) {
        console.log(retInfo)
        if(retInfo.success){
            ...
            var html = '';
            retInfo.data.forEach(function (value) {
                html += '<a onclick="nowilladd(this)" class="btn layui-btn-primary">'+value+'</a>'
            })
            $('.nowillUl p').html(html);
        }else{
            alert(retInfo.data);
        }
    },'请求失败', 'GET', undefined, undefined);
}
function nowilladd(obj) {
    $('.nowIll').val($(obj).text());
    $('.nowillUl').fadeOut();
}
function maitelladd(obj) {
    if($('.maintell').val().trim() == ''){
        $('.maintell').val($(obj).text())
    }else{
        $('.maintell').val($('.maintell').val() + ','+$(obj).text())
    }
}
...
function thatTimeChoose(obj){
    if($(obj).attr('valuezi') == 'model'){
        if($(obj).hasClass('sureTime') != true){
            getModelNo(form,$(".fenToDoctor").find("option:selected").attr('title'));
        }
    }else if($(obj).attr('valuezi') == 'medicalmodel'){
        if($(obj).hasClass('sureTime') != true){
            getMedicalModelNo(form,$(".fenToDoctor").find("option:selected").attr('title'));
        }
    }
    $('.thatTimeChoose').css('background-color','white').css('color','#222').css('border','1px solid #e7e7e7').removeClass('sureTime');
    $(obj).css('background-color','#009688').css('color','white').addClass('sureTime');
};
//编辑医生初始化坐诊表格
...
function init_img_phpto(file,num){
//	console.log(file);
    var html = '<div class="initImg" style="border: 1px solid #dddddd" data_id="0" data_val="0">' +
            '<img style="height: inherit" onclick="delphotoPiliang(this);stopEvt(event)" class="closePhoto" src="img/closePhoto.png">' +
            '<img class="photoImg" onclick="readBigPhoto()" src="'+file+'"></div>'

    $(".imgList" + num).append(html);

}
function readBigPhoto() {
    $('.fs_gallery').remove();
    $('#gallery img').not(".closePhoto").fsgallery();
}
...
function init_getimg(file){
//	console.log(file);
    var html="";
    for (var i = 0; i < file.length; i++) {
        html = html + '<div class="initImg " style="text-align: center;border: 1px solid #dddddd;margin-top: 10px;" data_id="0" data_val="' + i + '">' +
            '<a href="' + file[i].imgUrl + '" class="swipebox" title="拍片图片"> ' +
            '<img src="' + file[i].imgUrl + '" alt="拍片图片"> ' +
            '</a><input type="text" style="background-color: #efefef"  class="layui-input remark" placeholder="" disabled  value="' + file[i].remark + '"></div>'
    }
    $(".imgList").html(html);
    $( '.swipebox' ).swipebox();
}
/*for (var i = 0; i < file.length; i++) {
    html = html + '<div class="initImg " style="text-align: center;border: 1px solid #dddddd;margin-top: 10px;" data_id="0" data_val="' + i + '">' +
        '<img class="imgPhoto"  layer-src="' + file[i].imgUrl + '"  alt="图片名" src="' + file[i].imgUrl + '"><input type="text" style="background-color: #efefef"  class="layui-input remark" placeholder="" disabled  value="' + file[i].remark + '"></div>'
}*/
    ...
    var ajaxParam =
        {
            ...
            error:function (dataStr,status,readyState) {
                console.log(44)
                layui.layer.alert('系统繁忙，请稍后再试',{icon:5})
