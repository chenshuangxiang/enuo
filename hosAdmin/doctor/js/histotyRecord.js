layui.use(['form','layer','jquery','element'],function(){
    ...
    $('.recordSelect').change(function () {
        console.log($('.recordSelect').val())
        if ($('.recordSelect').val() == '回访状态') {
            $('.news_content tr').show();
        } else {
            $('.recordStatusTd').each(function () {
                console.log($(this).text())
                if ($('.recordSelect').val() == $(this).text()) {
                    $(this).parent().show();
                } else {
                    $(this).parent().hide();
                }
    ...
    function renderDateDiagnosis(data){
        ...
        if(data && data.length != 0){
            for(var i=0;i<data.length;i++){
                ...
                if(data[i].diseaseCourseRecordCount > 0){
                    dataHtml+='<td style="color: #2299ee;cursor: pointer;" onclick="diseaseCourseRecordOpen(this)" valueid="'+data[i].id+'" valuecount="'+data[i].diseaseCourseRecordCount+'">查看</td>';
                }else{
                    dataHtml += '<td>无</td>'
                }
        ...
        if(data && data.length != 0){
            for(var i=0;i<data.length;i++){
                dataHtml += '<tr>'
                    ...
                    +'<td class="recordStatusTd">'+recordStatusHospital(data[i].status)+'</td>';
                ...
                dataHtml +='<td>'+noData(data[i].purpose)+'</td>'
    ...
    $("body").on("click",".news_record_add",function(){
        ...
        var index = layui.layer.open({
            ...
            title : "患者回访信息添加",
            type : 2,
            content : "recordAdd.html?id=" +getQueryString('id'),
