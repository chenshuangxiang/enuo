        ...
        if(data.length != 0){
            ...
            for(var i=0;i<data.length;i++){
                ...
                if(data[i].state == 'normal'){
                    ...
                    dataHtml +='<td>退单待审核</td>';
                }else if(data[i].state == 'auditFail'){
                    dataHtml +='<td>退单被驳回</td>';
                }
                if(data[i].state != 'audit'){
                    dataHtml +='<td><a onclick="chargeback(this)" style=" height: 26px; line-height: 26px; padding: 0 8px;" userOfflineProjectId="'+data[i].userOfflineProject+'" projectMaterialId="'+data[i].id+'" orderId="'+data[i].orderId+'" class="layui-btn layui-btn-danger layui-btn-xs">退单</a></td>'
                }else{
                    dataHtml +='<td></td>'
                }
                dataHtml +='</tr>';
