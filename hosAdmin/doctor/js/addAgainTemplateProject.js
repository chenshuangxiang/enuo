...
/*    setTimeout(function () {
  ...
  /*  form.on('select(projectName)', function(data){  //品名获取品名信息
    ...
    form.render();
    //品名信息
    function getModelNoInfo(form,id,Url,medicalInsurance) {
        var id = id;
        var url = SERVER_ADDR + Url;
        var Data = {};
        Data.id = id;
        ...
        ajaxGetRetInfo(url,Data,function (retInfo) {
            ...
            if(retInfo.success){
                //选完品名进行存储
                var addCailiao = {};
                addCailiao.hospitalMaterialTemplateId =  id;
   ...
   /* form.on('select(cailiaoName)', function(data){
    ...
    form.on("submit(addProjectPost)",function(data){
        var postData = {};
        postData.userOfflineProjectId = getQueryString('valueid');
        postData.map = getNewMaterialProject(tablePostdata[0]);
        postData.medicalInsuranceMap = getNewMedicalcareProject(tablePostdata[0]);
       /* postData.diseases = $('.combo-input').val();
        ...
        var url = SERVER_ADDR + "/hospital/doctor/index/addNewProject";
...
function getNewMaterialProject(m){
    var familiesList = [];
    m.forEach(function (value) {
        console.log(value)
        var oneFamilies = {};
        if(value.medicalInsurance == '否'){
            oneFamilies.hospitalMaterialTemplateId = value.hospitalMaterialTemplateId;
            oneFamilies.quantity = value.quantity;
            oneFamilies.price = value.price;
            familiesList.push(oneFamilies);
        }
    });
    return JSON.stringify(familiesList);
}
function getNewMedicalcareProject(m){
    var familiesList = [];
    m.forEach(function (value) {
        var oneFamilies = {};
        if(value.medicalInsurance == '是'){
            oneFamilies.materialMedicalcareTemplateId = value.hospitalMaterialTemplateId;
            oneFamilies.quantity = value.quantity;
            oneFamilies.price = value.price;
            familiesList.push(oneFamilies);
        }
