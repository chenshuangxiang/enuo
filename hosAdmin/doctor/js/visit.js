...
layui.use(['form','layer','jquery','laypage','laydate'],function(){
    ...
    $('.appointmentList').addClass('layui-this').parent().parent().addClass('layui-nav-itemed');
    if(localStorage.getItem('loginEnter') == 0 && localStorage.getItem('todayNeedAccessCount') != 0) { //0代表刚登录进来
        layer.confirm('您今日有 <span style="color: #FF5722">' + localStorage.getItem('todayNeedAccessCount') + '</span> 条数据需要回访', {
            btn: ['去回访','暂不回访'], //按钮
            icon:6
        }, function(){
            localStorage.setItem('loginEnter',1);
            $('.todayRecord').click();
            window.location.href = SERVER_ADDR + '/hosAdmin/doctor/index.html#todayAllotRecord.html?v=1134'
        }, function(){
            localStorage.setItem('loginEnter',1);
        });
    }
    ...
    $('.sureFenTo').click(function () {
        ...
        var data = {};
        var url = SERVER_ADDR + "/hospital/doctor/index/allot";
        data.visitRecordId =elThisid;
...
function searchBtn(pageNumber) {
    ...
    var url = SERVER_ADDR + "/hospital/doctor/index/getVisitRecordList.json";
    var data = {};
    ...
    if(data.length != 0){
        for(var i=0;i<data.length;i++){
            ...
            if(data[i].repeat == true){
                if(data[i].status == "complete" && !data[i].userOffLineProjectId){
                    dataHtml += '<td style="color: #2299ee;cursor: pointer" valueid="'+data[i].id+'" valuestatus="complete" onclick="fuzhen(this)">复诊</td>'
                }else{
                    dataHtml += '<td>复诊</td>'
                }
            ...
            dataHtml +=  '<td>';
                if(data[i].status == "wait"){
                    if(data[i].repeat == true){
                        dataHtml += '<a class="layui-btn layui-btn-primary layui-btn-mini" onclick="fuzhen(this)" valueid="'+data[i].id+'" >复诊</a>' ;
                    }
                    if(!data[i].doctor){
                        dataHtml += '<a class="layui-btn layui-btn-normal layui-btn-mini firm" onclick="fenVisitTo(this)" valueid="'+data[i].id+'" >分诊</a>' ;
                    }
                    dataHtml += '<a class="layui-btn layui-btn-mini firm" onclick="addVisitProject(this)" valueid="'+data[i].id+'" doctorid="'+data[i].doctorId+'" >治疗</a>';
                    if(data[i].type == "product" || data[i].type == "coupon"){
                        dataHtml +=  '<a class="layui-btn layui-btn-normal layui-btn-mini" onclick="visit_firm(this)" valueid="'+data[i].id+'">完成</a>';
                    }
                    dataHtml +=  '<a class="layui-btn layui-btn-danger layui-btn-mini" onclick="getNoSuccessOpen(this)" valueid="'+data[i].id+'">到诊未成交</a><br>';
                    dataHtml +=  '<a class="layui-btn layui-btn-danger layui-btn-mini" onclick="visit_close(this)" valueid="'+data[i].id+'">弃诊</a>';
                    dataHtml += '<a class="layui-btn layui-btn-danger layui-btn-mini" title="删除记录" onclick="delVisitTo(this)" valueid="'+data[i].id+'"><i class="layui-icon" style="margin-right: 0;font-size: 19px !important;">&#xe640;</i></a>' ;
                ...
                //dataHtml +=  '<a class="layui-btn layui-btn-normal layui-btn-mini firm" onclick="reset_fen_appiontmentList_open(this)" valueid="'+data[i].id+'" visitrecordid="'+data[i].visitRecordId+'" doctorid="'+data[i].doctorId+'" doctorhelpmateid="'+data[i].doctorHelpmateId+'">添加项目</a>'
                    ...
                    dataHtml += '<i class="layui-icon" onclick="layui.layer.msg('+"'就诊已完成'"+')" style="color: #0aba07;margin-right: 0;font-size: 19px !important;">&#xe605;</i>' ;
                }else if(data[i].status == "leave"){
                    dataHtml += '<i class="layui-icon" onclick="layui.layer.msg('+"'用户已弃诊'"+')" style="margin-right: 0;font-size: 19px !important;">&#x1006;</i>' ;
                }else if(data[i].status == "No_DEAL_WAS_CONCLUDED_AT_THE_CLINIC"){
                    dataHtml += '<a class="layui-btn layui-btn-danger layui-btn-mini" title="删除记录" onclick="delVisitTo(this)" valueid="'+data[i].id+'"><i class="layui-icon" style="margin-right: 0;font-size: 19px !important;">&#xe640;</i></a>' ;
                }
            //if(data[i].doctor){
                dataHtml += '<a class="layui-btn layui-btn-normal layui-btn-mini" title="病程记录" onclick="diseaseCourseRecordOpen(this)" valueid="'+data[i].id+'" valueCount="'+data[i].diseaseCourseRecordCount+'"><i class="layui-icon" style="margin-right: -2px;margin-left:-2px;font-size: 18px !important;">&#xe642;</i></a>' ;
            //}
            dataHtml += '</td>'
                +'</tr>';
        }
...
function addVisitProject(obj) {
    ...
    var index = layui.layer.open({
        ...
        type : 2,
        area:['950px','92%'],
        content : "addTemplateProject.html?valueid=" + elThis + '&doctorid=' + doctorId + '&v=1134',
        success : function(layero, index){
            localStorage.setItem('visit',$('.layui-laypage-curr em').eq(1).text());
...
function visit_firm(obj) {
    ...
    layer.confirm('确认该项目已完成？', {
        ...
        postget_ajax("/hospital/doctor/index/complete",data,'POST',function(res){
            if(res.success){
...
function delVisitTo(obj) {
    ...
    layer.confirm('确认删除？', {
        ...
        postget_ajax("/hospital/reception/visitRecord/delete",data,'POST',function(res){
            if(res.success){
//				window.location.href = "appointment_list.html";
...
function diseaseCourseRecordOpen(obj) {
    var elThis = $(obj).attr('valueid');
    var elThisCount = $(obj).attr('valueCount');
    var content = "diseaseCourseRecord.html?valueid=" + elThis + '&valuecount='+ elThisCount + '&v=1134';
    var index = layui.layer.open({
        title : "病程记录",
        type : 2,
        area:['800px','600px'],
...
function getNoSuccessOpen(obj) {

    var elThis = $(obj).attr('valueid');
    var content = "getNoSuccess.html?valueid=" + elThis + '&v=1134';
    var index = layui.layer.open({
        title : "到诊未成交",
        type : 2,
        area:['600px','400px'],
