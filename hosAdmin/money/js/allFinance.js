...
layui.use(['form','layer','jquery','laypage','laydate'],function(){
    ...
    searchBtn(localStorage.getItem('allFinance') || 1);
    localStorage.removeItem('allFinance');
    $('.allFinance').addClass('layui-this').parent().parent().addClass('layui-nav-itemed');
	//加载页面数据
...
function searchBtn(pageNumber) {
    ...
    var url = SERVER_ADDR + "/hospital/finance/getList.json";
    var data = {};
        ...
        for(var i=0;i<data.length;i++){
            ...
            dataHtml += '<tr>'
              ...
              /*  +'<td>'+data[i].reservationDate+'</td>'*/
                +'<td style="color: #2299ee;cursor: pointer" valuemobile="'+data[i].mobile+'" valueid="'+data[i].hospitalUserId+'" onclick="getUserDetail(this)">'+data[i].userName+'</td>'
                +'<td>'+data[i].mobile+'</td>';
           ...
           /* if(data[i].payMethod == 'cash'){
            ...
            dataHtml +=  '<a class="layui-btn layui-btn-normal layui-btn-mini" onclick="printOrderOpen(this)"  valueid="'+data[i].id+'" payMethod="'+data[i].payMethod+'">打单</a>'
