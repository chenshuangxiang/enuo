        ...
        for(var i=0;i<data.length;i++){
            ...
            dataHtml += '<tr>'
              ...
              /*  +'<td>'+data[i].reservationDate+'</td>'*/
                +'<td style="color: #2299ee;cursor: pointer" valuemobile="'+data[i].mobile+'" onclick="visitUserInfo(this)">'+data[i].fullName+'</td>'
                +'<td>'+data[i].mobile+'</td>';
...
function addVisitProject(obj) {
    ...
    var index = layui.layer.open({
        ...
        type : 2,
        area:['890px','92%'],
        content : "addTemplateProject.html?valueid=" + elThis,
        success : function(layero, index){
            localStorage.setItem('visit',$('.layui-laypage-curr em').eq(1).text());
