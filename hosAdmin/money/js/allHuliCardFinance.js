...
layui.use(['form','layer','jquery','laypage','laydate'],function(){
    ...
    searchBtn(localStorage.getItem('allHuliCardFinance') || 1);
    localStorage.removeItem('allHuliCardFinance');
    $('.allHuliCardFinance').addClass('layui-this').parent().parent().addClass('layui-nav-itemed');
	//加载页面数据
...
function searchBtn(pageNumber) {
    ...
    var url = SERVER_ADDR + "/hospital/finance/getNursingCardList.json";
    var data = {};
...
function printOrderOpen(obj) {
  ...
  /*  var payMethods = $(obj).attr('payMethod');
    ...
    var index = layui.layer.open({
        ...
        type : 2,
        area: ['750px','80%'],
        content : 'sureHuliCardOrderPrint.html?id=' + id + '&v=1134',
        success : function(layero, index){
