...
layui.use(['form','layer','jquery','laypage','laydate','element'],function(){
    ...
    $('.yuyueSkill').addClass('layui-this').parent().parent().addClass('layui-nav-itemed');
...
function searchBtn(pageNumber,dateType) {
    ...
    var url = SERVER_ADDR + "/hospital/check/getList.json";
    var data = {};
...
function renderDate(data){
    ...
    if(data.length != 0){
        for(var i=0;i<data.length;i++){
            ...
            dataHtml += '<tr>'
                +'<td>'+Number(i+1)+'</td>'
                +'<td style="color: #2299ee;cursor: pointer" valuemobile="'+data[i].mobile+'" onclick="Get.userInfo(this)">'+noData(data[i].userName)+'</td>'
            ...
            dataHtml +=  '<td><a class="layui-btn layui-btn-normal layui-btn-mini" onclick="addPhotos(this)" valueid="'+data[i].id+'">上传图片</a></td>'
           /* if(data[i].status == '候诊'){
                dataHtml += '<td><a class="layui-btn layui-btn-normal layui-btn-mini" onclick="reset_fen_open(this)" valueid="'+data[i].id+'"  doctorid="'+data[i].doctorId+'" doctorhelpmateid="'+data[i].doctorHelpmateId+'"><i class="iconfont icon-edit"></i>修改分诊</a>';

            }else if(data[i].status == '治疗中'){
                dataHtml +=  '<td><a class="layui-btn layui-btn-danger layui-btn-mini" onclick="visit_close(this)" valueid="'+data[i].id+'">取消</a><a class="layui-btn layui-btn-normal layui-btn-mini" onclick="reset_addwrite(this)" valueid="'+data[i].id+'"><i class="layui-icon" style="vertical-align: middle;margin-right: 0">&#xe60b;</i></a></td>'
            }else{
                dataHtml += '<td><a class="layui-btn layui-btn-normal layui-btn-mini" onclick="reset_addwrite(this)" valueid="'+data[i].id+'"><i class="layui-icon" style="vertical-align: middle;margin-right: 0">&#xe60b;</i></a>';
            }*/
            dataHtml+='</tr>'
        }
    }else{
...
function addPhotos(obj) {
    ...
    var index = layui.layer.open({
        ...
        type : 2,
        area: ['85%','80%'],
        content : "addPhotos.html?valueid=" + id,
