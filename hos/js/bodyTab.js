...
layui.define(["element","jquery"],function(exports){
		...
		// $(".layui-nav .layui-nav-item a").on("click",function(){
			if(_this.find("i.iconfont,i.layui-icon").attr("data-icon") != undefined){
				...
				if(that.hasTab(_this.find("cite").text()) == -1 && _this.siblings("dl.layui-nav-child").length == 0){
					...
					console.log('1的tab值'+tabIdIndex);
                    $('.layui-tab .layui-tab-content > div').eq(tabIdIndex).children('iframe')[0].contentWindow.location.reload();
				}else{
					//当前窗口内容
					var curmenu = {
						"icon" : _this.find("i.iconfont").attr("data-icon")!=undefined ? _this.find("i.iconfont").attr("data-icon") : _this.find("i.layui-icon").attr("data-icon"),
                    ...
                    var l = ".layui-tab-title"
                        , e = $(".layui-tab[lay-filter=" + tabFilter + "]")
                        , n = e.children(l)
                        , s = n.find('>li[lay-id="' + that.getLayId(_this.find("cite").text()) + '"]');
                    console.log('element'+s.index())
                    $('.layui-tab .layui-tab-content > div').eq(s.index()).children('iframe')[0].contentWindow.location.reload();
                    console.log('2的tab值'+_this.parent().index());
					//$('.layui-tab .layui-tab-content > div').eq(tabIdIndex).children('iframe')[0].contentWindow.location.reload();
                }
			}
		// })
	...
	$("body").on("click",".top_tab li",function(){
        ...
        console.log('3的li的tab值'+$(this).index());
        $('.layui-tab .layui-tab-content > div').eq($(this).index()).children('iframe')[0].contentWindow.location.reload();
	})
