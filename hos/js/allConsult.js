layui.use(['form','layer','jquery','laypage','laydate','element'],function(){
   ...
   /* form.on('select(newsHos)', function(data){  //根据医院获取病种
	...
	function searchBtn(pageNumber) {
        ...
        var url = SERVER_ADDR + "/admin/advisory/getList.json";
        var data = {};
            ...
            for(var i=0;i<data.length;i++){
               ...
               /* if(data[i].status == "unallot"){
                ...
                dataHtml +=  /*'<a class="layui-btn layui-btn-normal layui-btn-mini news_record_add" style="display: none" valueid="'+data[i].id+'"><i class="layui-icon">&#xe650;</i> 回访</a>' +*/
                    '</td>' +
                    '<td><a class="layui-btn layui-btn-normal layui-btn-mini news_reset_dohis" valueid="'+data[i].id+'"  valuemobile="'+data[i].mobile+'" valueusername="'+data[i].name+'"><i class="layui-icon" style="font-size: 20px !important;">&#xe60a;</i></a>' +
                    '<a class="layui-btn layui-btn-normal layui-btn-mini news_reset_recordhis" valueid="'+data[i].id+'" patientid="'+data[i].patientId+'" valuemobile="'+data[i].mobile+'" valueusername="'+data[i].name+'">回访</a></td>'
                    +'</tr>';
            }
        }else{
