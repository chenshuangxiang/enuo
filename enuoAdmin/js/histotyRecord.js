...
layui.use(['form','layer','jquery','element'],function(){
	...
	function searchBtn(pageNumber) {
        ...
        var url = getUrl() + "/admin/user/getHistoryRecord.json";
        var data = {};
        ...
        if(data && data.length != 0){
            for(var i=0;i<data.length;i++){
                ...
                if(data[i].userOfflineProjectId){
                    dataHtml += '<td style="color:#00a2d4;cursor: pointer;" data_id="'+data[i].visitRecordId+'" onclick="orderOpen(this)">'+data[i].projectName+'</td>'
                }else{
                    dataHtml +='<td>'+data[i].projectName+'</td>'
                }
                if(returnProjectType(data[i].orderType) == '中奖项目'){
                    dataHtml +='<td>0.00</td>'
                }else{
                    dataHtml +='<td>'+data[i].amount+'</td>'
                }
                dataHtml += '<td style="word-break:break-all;word-wrap:break-word">'+returnProjectNumber(data[i].number)+'</td>'
                if(data[i].orderType == 'offline'){
                    dataHtml += '<td>'+returnProjectVisitRecordStatus(data[i].visitRecordStatus)+'</td>';
                }else{
                    if (data[i].projectName == '美丽回家大礼包') {
                        dataHtml += '<td>'+returnProjectOrderStatus(data[i].status)+'</td>';
                    }else if(data[i].orderType == 'PRIZE' && data[i].status == 'waitPaid'){
                        dataHtml += '<td>未预约(已支付)</td>';
                    }else{
                        if(data[i].isReservation == true){
                            dataHtml += '<td>已预约(已支付)</td>';
                        }else if(data[i].isReservation == false){
                            if(data[i].prizeInvalid == true){
                                dataHtml += '<td style="color: #666">已失效</td>';
                            }else{
                                dataHtml += '<td>未预约(已支付)</td>';
                            }
                        }else{
    ...
    function getPersonInfo() {
            ajaxGetRetInfo(getUrl() + '/customerService/getAdvisoryDetail.json',{id:getQueryString('valueid')},function (retInfo) {
                console.log(retInfo)
                if(retInfo.success){
    ...
    form.on("submit(resetInfoBtn)",function(data){
        ...
        console.log(addNews);
            ...
            var url = getUrl() + "/customerService/updateAdvisoryPatient";
...
function searchTab(type) {
    var type = type;
    var url = getUrl() + "/admin/user/getPatientAccessDetail.json";
    var data = {};
    data.mobile = getQueryString('mobile');
    data.type = type;
    ajaxGetRetInfo(url,data,function (retInfo) {
...
var Get = {
    firstConcult: function () {
            var url = getUrl() + '/admin/user/getPatientInfo.json';
            var Data = {};
            Data.mobile = getQueryString('mobile');
    ...
    firstConcultSuccess: function (retInfo) {
        ...
        if (retInfo.success == true) {
            ...
            $('.name').text(getQueryString('username'));
    ...
    resetRecord: function (obj) {
        var elThis = $(obj);
        var url = getUrl() + '/customerService/updateAccess';
        var Data = {};
