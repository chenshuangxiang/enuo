...
function gulpScripts(app_name) {
    return gulp.src([app_name + '/**/*.js']) //源文件下的所有js
        ...
        .pipe(gulp.dest("hosAdmin_upload"));//复制到目标文件路径
}
...
function gulpStyles(app_name) {
    return gulp.src([app_name + '/**/*.css'])
        .pipe(assetRev())
        .pipe(minifyCss())
        .pipe(gulp.dest("hosAdmin_upload"));
}
function gulpImages(app_name) {
    return gulp.src([app_name + '/**/img/*',app_name + '/**/img/**/*',app_name + '/**/*.png',app_name + '/**/*.jpg',app_name + '/**/*.gif'])  //,app_name + '/!**/option/!*'
        //.pipe(imageMin({progressive: true}))
        .pipe(gulp.dest("hosAdmin_upload"));   //复制所有图片到目标文件夹
}
...
function gulpRevHtml(app_name) {
    gulp.src([app_name + '/*.html', app_name + '/**/*.html'])   //源文件下面是所有html
        ...
        .pipe(gulp.dest('hosAdmin_upload'));                   //打包到目标文件夹路径下面
}

gulp.task('app_scripts', function(){
    gulpScripts("hosAdmin");
});
...
gulp.task('app_rev', ['app_styles', 'app_scripts'], function(){
    gulpRevHtml("hosAdmin");
});
gulp.task('clean', del.bind(null, ['hosAdmin_upload'], {
    force: true
}));
